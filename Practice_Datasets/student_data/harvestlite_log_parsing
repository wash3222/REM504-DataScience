...
logs.read <- function(filepath,
  ...
  files <- list.files(path = filepath,
                      ...
                      ignore.case = TRUE,
                      full.names = TRUE)
  # Create a list of all the data frames! This runs log.read() on each file and gives us a list of lists of data frames
  data <-lapply(files, log.read)
  
  # For each of the data frames (e.g. metadata), go through the lists and grab then combine all the data frames with that name
  # This will give us a single list with one of each of the data frames containing all the values from the data frames with the same name
  ...
  # Just name those data frames with the correct names. We did them in the order they appear in data[[1]] so we can use that as the name vector
