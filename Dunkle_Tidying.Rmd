...
colnames(mydata_filtermass) = c("FilterNo","FilterMass","FilterSize")
mydata_filtermass
```
...
mydata_AFDM_long$TreatmentB = rep(TreatmentB, times = 2)
mydata_AFDM_long
usedfilters_mass = subset(mydata_filtermass, FilterNo == c(1:36))
mydata_AFDM_long = cbind(mydata_AFDM_long, usedfilters_mass)

mydata_AFDM_long$AFDM = ((as.numeric(mydata_AFDM_long$DryMass) - as.numeric(mydata_AFDM_long$FilterMass))-(as.numeric(mydata_AFDM_long$AshedMass) - as.numeric(mydata_AFDM_long$FilterMass)))/51.613
```


```{r, echo=F}
ggplot(mydata_AFDM_long, aes(x=TreatmentB, y=AFDM, fill=TreatmentA))+geom_boxplot()+
  scale_fill_grey()+
  labs(x=NULL, y="AFDM (mg/cm^2)")+theme_cowplot()+theme(legend.title=element_blank())
```