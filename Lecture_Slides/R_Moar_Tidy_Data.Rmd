...
# using - will give you all but. All rows but row 1. 
tidy.data[-1,]
...
# can aslo use double brackets This is useful if you title a column as a variable 
# col <- "treatment" 
# tidy.data$col will fail but tidy.data[[col]] will work 
# don't use attach. example: attach(tidy.data) 
...
dens.data %>% summarise(mean=mean(density))
# mean is whatever name you want to give it. 
...
dens.by.treat <- dens.data %>% group_by(treatment) %>% summarise(mean=mean(density),se=sd(density),n=n())
dens.by.treat
```
